...
impl Engine {
    ...
    fn material_hueristic(&self, board: &Board) -> Score {
        ...
        cp += ((board.pieces(Piece::Pawn) & *mine).0.count_ones()) as i16;
        cp -= ((board.pieces(Piece::Pawn) & *theirs).0.count_ones()) as i16;

        cp += ((board.pieces(Piece::Knight) & *mine).0.count_ones() * 3) as i16;
        cp -= ((board.pieces(Piece::Knight) & *theirs).0.count_ones() * 3) as i16;

        cp += ((board.pieces(Piece::Bishop) & *mine).0.count_ones() * 3) as i16;
        cp -= ((board.pieces(Piece::Bishop) & *theirs).0.count_ones() * 3) as i16;

        cp += ((board.pieces(Piece::Rook) & *mine).0.count_ones() * 5) as i16;
        cp -= ((board.pieces(Piece::Rook) & *theirs).0.count_ones() * 5) as i16;

        cp += ((board.pieces(Piece::Queen) & *mine).0.count_ones() * 9) as i16;
        cp -= ((board.pieces(Piece::Queen) & *theirs).0.count_ones() * 9) as i16;
